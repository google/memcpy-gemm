...
template <>
bool FillArray<half>(half *A, int n, absl::BitGen *rng, float scale,
                     bool nv_gauss) {
  ...
  if (nv_gauss) {
    ...
    FillUniform<float>(absl::Span<float>(baseMatrix.get(), kBaseMatrixSize), rng,
                       scale);
  }
  for (int i = 0; i < n; i++) {
...
bool RandomMatrix<T>::Initialize(absl::BitGen *rng, float scale,
  ...
  return FillArray<T>(host_memory_, dim_size_m_ * dim_size_k_, rng, scale,
                      nv_gauss);
}
